		...
		if(json.status==1 && json.data){
			...
			if(json.data.spec0 || json.data.spec1){
				...
				html.push('<div id="specSaleHead" class="spec-head">销售规格</div>',
				          ...
				          '     <th>市场价<font color="red">*</font><br/><input type="text" class="spec-sale-ipt" onblur="WST.limitDecimal(this,2);batchChange(this.value,\'marketPrice\')" onkeypress="return WST.isNumberdoteKey(event)" onkeyup="javascript:WST.isChinese(this,1)"></th>',
				          '     <th>本店价<font color="red">*</font><br/><input type="text" class="spec-sale-ipt" onblur="WST.limitDecimal(this,2);batchChange(this.value,\'specPrice\')" onkeypress="return WST.isNumberdoteKey(event)" onkeyup="javascript:WST.isChinese(this,1)"></th>',
...
function saleByPage(p){
    ...
    var cols = [
        ...
        {title:'收藏数', name:'collectNum', width: 30},
...
function storeByPage(p){
    ...
    var cols = [
        ...
        {title:'收藏数', name:'collectNum', width: 40},
...
function illegalByPage(p){
    ...
    var cols = [
        ...
        {title:'违规原因', name:'illegalRemarks', width: 350},
...
function loadMould(json){
	...
	if(specAttrObj.spec0){
		for(var i=0;i<specAttrObj.spec0.length;i++){
		   if(specAttrObj.spec0[i].catId==json.data.spec0.catId)addSpecImg({catId:specAttrObj.spec0[i].catId,checked:'checked',val:specAttrObj.spec0[i].itemName,itemId:specAttrObj.spec0[i].itemId,specImg:specAttrObj.spec0[i].itemImg});
		}
	}
